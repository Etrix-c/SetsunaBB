...
// END Merge Post
					...
					do
					{
						...
						if (!isset($notification_emails[$cur_subscriber['language']]))
						{
							if (file_exists(PUN_ROOT.'lang/'.$cur_subscriber['language'].'/mail_templates/new_reply.tpl'))
							{
								...
								$mail_message = str_replace('<post_url>', get_base_url().'/posts.php?pid='.$new_pid.'#p'.$new_pid, $mail_message);
								$mail_message = str_replace('<unsubscribe_url>', get_base_url().'/posts.php?id='.$tid.'#unsubscribe', $mail_message);
								...
								$mail_message_full = str_replace('<post_url>', get_base_url().'/posts.php?pid='.$new_pid.'#p'.$new_pid, $mail_message_full);
								$mail_message_full = str_replace('<unsubscribe_url>', get_base_url().'/posts.php?id='.$tid.'#unsubscribe', $mail_message_full);
					...
					do
					{
						...
						if (!isset($notification_emails[$cur_subscriber['language']]))
						{
							if (file_exists(PUN_ROOT.'lang/'.$cur_subscriber['language'].'/mail_templates/new_topic.tpl'))
							{
								...
								$mail_message = str_replace('<topic_url>', get_base_url().'/posts.php?id='.$new_tid, $mail_message);
								$mail_message = str_replace('<unsubscribe_url>', get_base_url().'/forums.php?id='.$cur_posting['id'].'#unsubscribe', $mail_message);
								$mail_message = str_replace('<board_mailer>', $pun_config['o_board_title'], $mail_message);
								...
								$mail_message_full = str_replace('<message>', $cleaned_message, $mail_message_full);
								$mail_message_full = str_replace('<topic_url>', get_base_url().'/posts.php?id='.$new_tid, $mail_message_full);
								$mail_message_full = str_replace('<unsubscribe_url>', get_base_url().'/forums.php?id='.$cur_posting['id'].'#unsubscribe', $mail_message_full);
								$mail_message_full = str_replace('<board_mailer>', $pun_config['o_board_title'], $mail_message_full);
		...
		redirect('posts.php?pid='.$new_pid.'#p'.$new_pid, $lang_post['Post redirect']);
	}
}
...
<div class="linkst">
	<div class="inbox">
		<ul class="crumbs">
			...
			<li><span>»&#160;</span><a href="forums.php?id=<?php echo $cur_posting['id'] ?>"><?php echo pun_htmlspecialchars($cur_posting['forum_name']) ?></a></li>
<?php if (isset($_POST['req_subject'])): ?>			<li><span>»&#160;</span><?php echo pun_htmlspecialchars($_POST['req_subject']) ?></li>
<?php endif; ?>
<?php if (isset($cur_posting['subject'])): ?>			<li><span>»&#160;</span><a href="posts.php?id=<?php echo $tid ?>"><?php echo pun_htmlspecialchars($cur_posting['subject']) ?></a></li>
