...
<?php
		...
		{
			$forum = '<a href="forums.php?id='.$cur_search['forum_id'].'">'.pun_htmlspecialchars($cur_search['forum_name']).'</a>';
			...
			if ($show_as == 'posts')
			{
				...
				if ($cur_search['poster_id'] > 1)
				{
					if ($pun_user['g_view_users'] == '1')
						$pposter = '<strong><a href="showuser.php?id='.$cur_search['poster_id'].'">'.$pposter.'</a></strong>';
					else
...
<div class="blockpost<?php echo ($post_count % 2 == 0) ? ' roweven' : ' rowodd' ?><?php if ($cur_search['pid'] == $cur_search['first_post_id']) echo ' firstpost' ?><?php if ($post_count == 1) echo ' blockpost1' ?><?php if ($item_status != '') echo ' '.$item_status ?>">
	<h2><span><span class="conr">#<?php echo ($start_from + $post_count) ?></span> <span><?php if ($cur_search['pid'] != $cur_search['first_post_id']) echo $lang_topic['Re'].' ' ?><?php echo $forum ?></span> <span>»&#160;<a href="posts.php?id=<?php echo $cur_search['tid'].$url_shl ?>"><?php echo pun_htmlspecialchars($cur_search['subject']) ?></a></span> <span>»&#160;<a href="posts.php?pid=<?php echo $cur_search['pid'].$url_shl.'#p'.$cur_search['pid'] ?>"><?php echo format_time($cur_search['pposted']) ?></a></span></span></h2>
	<div class="box">
		<div class="inbox">
			<div class="postbody">
				<div class="postleft">
		...
		<div class="inbox">
			<div class="postfoot clearb">
				<div class="postfootright">
					<ul>
						<li><span><a href="posts.php?id=<?php echo $cur_search['tid'].$url_shl ?>"><?php echo $lang_search['Go to topic'] ?></a></span></li>
						<li><span><a href="posts.php?pid=<?php echo $cur_search['pid'].$url_shl.'#p'.$cur_search['pid'] ?>"><?php echo $lang_search['Go to post'] ?></a></span></li>
					</ul>
				</div>
			</div>
		</div>
...
<?php
			...
			else
			{
				...
				$subject = '<a href="posts.php?id='.$cur_search['tid'].$url_shl.'">'.pun_htmlspecialchars($cur_search['subject']).'</a> <span class="byuser">'.$lang_common['by'].' '.pun_htmlspecialchars($cur_search['poster']).'</span>'; // search HL - Visman
				...
				if (!$pun_user['is_guest'] && $cur_search['last_post'] > $pun_user['last_visit'] && (!isset($tracked_topics['topics'][$cur_search['tid']]) || $tracked_topics['topics'][$cur_search['tid']] < $cur_search['last_post']) && (!isset($tracked_topics['forums'][$cur_search['forum_id']]) || $tracked_topics['forums'][$cur_search['forum_id']] < $cur_search['last_post']))
				{
					...
					$subject_new_posts = '<span class="newtext">[ <a href="posts.php?id='.$cur_search['tid'].'&amp;action=new" title="'.$lang_common['New posts info'].'">'.$lang_common['New posts'].'</a> ]</span>';
				}
				else
				...
				if ($num_pages_topic > 1)
					$subject_multipage = '<span class="pagestext">[ '.paginate($num_pages_topic, -1, 'posts.php?id='.$cur_search['tid'].$url_shl).' ]</span>';
				else
...
?>
				<tr class="<?php echo $item_status ?>">
					...
					<td class="tcr"><?php echo '<a href="posts.php?id='.$cur_search['tid'].($num_pages_topic > 1 ? '&p='.$num_pages_topic : '').$url_shl.'#p'.$cur_search['last_post_id'].'">'.format_time($cur_search['last_post']).'</a> <span class="byuser">'.$lang_common['by'].' '.pun_htmlspecialchars($cur_search['last_poster']) ?></span></td>
				</tr>
<?php
