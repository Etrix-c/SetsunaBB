...
if ($action === 'feed')
{
	...
	if (isset($_GET['tid']))
	{
		...
		$feed = array(
			...
			'link'			=>	get_base_url(true).'/posts.php?id='.$tid,
		...
		while ($cur_post = $db->fetch_assoc($result))
		{
			...
			$item = array(
				...
				'link'			=>	get_base_url(true).'/posts.php?pid='.$cur_post['id'].'#p'.$cur_post['id'],
			...
			if ($cur_post['poster_id'] > 1)
			{
				...
				$item['author']['uri'] = get_base_url(true).'/showuser.php?id='.$cur_post['poster_id'];
			}
		...
		if (!isset($feed) || $cache_expire < $now)
		{
			...
			while ($cur_topic = $db->fetch_assoc($result))
			{
				...
				$item = array(
					...
					'link'			=>	'/posts.php?id='.$cur_topic['id'].($order_posted ? '' : '&action=new'),
				...
				if ($cur_topic['poster_id'] > 1)
				{
					...
					$item['author']['uri'] = '/showuser.php?id='.$cur_topic['poster_id'];
				}
...
else if ($action === 'online' || $action === 'online_full')
{
	...
	while ($pun_user_online = $db->fetch_assoc($result))
	{
		...
		{
			$users[] = $pun_user['g_view_users'] == '1' ? '<a href="'.pun_htmlspecialchars(get_base_url(true)).'/showuser.php?id='.$pun_user_online['user_id'].'">'.pun_htmlspecialchars($pun_user_online['ident']).'</a>' : pun_htmlspecialchars($pun_user_online['ident']);
			++$num_users;
		}
		else
			++$num_guests;
...
else if ($action === 'stats')
{
	...
	echo sprintf($lang_index['Newest user'], (($pun_user['g_view_users'] == '1') ? '<a href="'.pun_htmlspecialchars(get_base_url(true)).'/showuser.php?id='.$stats['last_user']['id'].'">'.pun_htmlspecialchars($stats['last_user']['username']).'</a>' : pun_htmlspecialchars($stats['last_user']['username']))).'<br />'."\n";
