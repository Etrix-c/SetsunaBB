...
foreach ($sf_array[$sf_cur_forum] as $cur_subforum)
{
	...
	if (!empty($sf_array[$cur_subforum['fid']]))
	{
		...
		{
			$sf_list[] = '<a class="subforum_name" href="forums.php?id='.$cur['fid'].'">'.pun_htmlspecialchars($cur['forum_name']).'</a>';
		}
	}
...
<?php
	...
	{
		$forum_field = '<h3><a href="forums.php?id='.$cur_subforum['fid'].'">'.pun_htmlspecialchars($cur_subforum['forum_name']).'</a>'.(!empty($forum_field_new) ? ' '.$forum_field_new : '').'</h3>';
	...
	if ($cur_subforum['last_post'] != '')
	{
		...
		$last_post = '<a href="posts.php?pid='.$cur_subforum['last_post_id'].'#p'.$cur_subforum['last_post_id'].'">'.pun_htmlspecialchars(pun_strlen($cur_subforum['last_topic']) > 30 ? mb_substr($cur_subforum['last_topic'], 0, 30).'…' : $cur_subforum['last_topic']).'</a> <span class="byuser">'.format_time($cur_subforum['last_post']).' '.$lang_common['by'].' '.pun_htmlspecialchars($cur_subforum['last_poster']).'</span>'; // last topic on index - Visman
	}
