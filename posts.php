...
if ($pid)
{
	...
	if ($pun_user['is_guest'])
	{
		...
		header('Location: posts.php?id='.$id.($_GET['p'] > 1 ? '&p='.$_GET['p'] : '').($search_hl > 0 ? '&search_hl='.$search_hl : '').'#p'.$pid, true, 301);
		exit;
	}
}
else
{
	...
	if ($action === 'new')
	{
		if (!$pun_user['is_guest'])
		{
			...
			{
				header('Location: posts.php?pid='.$first_new_post_id.'#p'.$first_new_post_id);
				exit;
			}
		}
...
//		}
		...
		if (! empty($cur_topic))
		{
			...
			header('Location: posts.php?id='.$id.($last_page > 1 ? '&p='.$last_page : '').'#p'.$cur_topic['last_post_id']);
			exit;
		}
	}
}
...
if (!is_null(poll_post('poll_submit')))
{
	...
	redirect('posts.php?id='.$id.((isset($_GET['p']) && $_GET['p'] > 1) ? '&p='.intval($_GET['p']) : ''), $lang_poll['M0']);
}
...
if (isset($_GET['search_hl']))
{
	...
	else // запрос устарел или от другого юзера
	{
		if ($id > 0)
		{
			...
			header('Location: posts.php?id='.$id.$p.($pid > 0 ? '#p'.$pid : ''), true, 301);
		}
		else
			header('Location: posts.php?pid='.$pid.'#p'.$pid, true, 301);
...
$paging_links = '<span class="pages-label">'.$lang_common['Pages'].' </span>'.paginate($num_pages, $p, 'posts.php?id='.$id.$url_shl); // search HL - Visman
...
$page_head = array();
$page_head['canonical'] = '<link rel="canonical" href="posts.php?id='.$id.($p == 1 ? '' : '&amp;p='.$p).'" title="'.sprintf($lang_common['Page'], $p).'" />';

if ($num_pages > 1)
{
	if ($p > 1)
		$page_head['prev'] = '<link rel="prev" href="posts.php?id='.$id.($p == 2 ? '' : '&amp;p='.($p - 1)).'" title="'.sprintf($lang_common['Page'], $p - 1).'" />';
	if ($p < $num_pages)
		$page_head['next'] = '<link rel="next" href="posts.php?id='.$id.'&amp;p='.($p + 1).'" title="'.sprintf($lang_common['Page'], $p + 1).'" />';
}
...
<?php echo sf_crumbs($cur_topic['forum_id']); // MOD subforums - Visman ?>
			<li><span>»&#160;</span><a href="forums.php?id=<?php echo $cur_topic['forum_id'] ?>"><?php echo pun_htmlspecialchars($cur_topic['forum_name']) ?></a></li>
			<li><span>»&#160;</span><strong><a href="posts.php?id=<?php echo $id.$url_shl // search HL - Visman ?>"><?php echo pun_htmlspecialchars($cur_topic['subject']) ?></a></strong></li>
		</ul>
		<div class="pagepost">
...
<?php
	...
	if ($cur_post['poster_id'] > 1)
	{
		...
		if ($pun_user['g_view_users'] == '1')
			$username = '<a href="showuser.php?id='.$cur_post['poster_id'].'">'.pun_htmlspecialchars($cur_post['username']).'</a>';
		else
...
// New PMS - Visman
		...
		if ($pun_user['g_id'] == PUN_ADMIN || ($pun_user['g_moderator'] == '1' && $pun_user['g_mod_promote_users'] == '1'))
		{
			if ($cur_post['g_promote_next_group'])
				$user_info[] = '<dd><span><a href="showuser.php?action=promote&amp;id='.$cur_post['poster_id'].'&amp;pid='.$cur_post['id'].'&amp;csrf_hash='.csrf_hash().'">'.$lang_topic['Promote user'].'</a></span></dd>';
		}
...
<div id="p<?php echo $cur_post['id'] ?>" class="blockpost<?php echo ($post_count % 2 == 0) ? ' roweven' : ' rowodd' ?><?php if ($cur_post['id'] == $cur_topic['first_post_id']) echo ' firstpost'; ?><?php if ($post_count == 1) echo ' blockpost1'; ?>">
	<h2><span><span class="conr">#<?php echo ($start_from + $post_count) ?></span> <a href="<?php echo ($pun_user['is_guest'] ? '' : 'posts.php?pid='.$cur_post['id']).'#p'.$cur_post['id'] ?>"><?php echo format_time($cur_post['posted']) ?></a></span></h2>
	<div class="box">
		<div class="inbox">
			<div class="postbody">
				<div class="postleft">
...
<?php
	...
	{
		if ($pun_user['g_view_users'] == '1')
			$users[] = "\n\t\t\t\t".'<dd><a href="showuser.php?id='.$online_id.'">'.pun_htmlspecialchars($online_name).'</a>';
		else
