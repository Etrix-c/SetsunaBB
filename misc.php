...
else if ($action === 'markforumread')
{
	...
	redirect('forums.php?id='.$fid, $lang_misc['Mark forum read redirect']);
}
...
else if (isset($_GET['email']))
{
	...
	if (!isset($redirect_url))
		$redirect_url = get_base_url(true).'/showuser.php?id='.$recipient_id;
	else if (preg_match('%viewtopic\.php\?pid=(\d+)$%', $redirect_url, $matches))
		$redirect_url .= '#p'.$matches[1];
	...
	if (isset($_POST['form_sent']))
	{
		...
		if ($pun_config['o_report_method'] == '1' || $pun_config['o_report_method'] == '2')
		{
			...
			if ($pun_config['o_mailing_list'] != '')
			{
				...
				$mail_message = str_replace('<post_url>', get_base_url().'/posts.php?pid='.$post_id.'#p'.$post_id, $mail_message);
		...
		redirect('posts.php?pid='.$post_id.'#p'.$post_id, $lang_misc['Report redirect']);
	}
...
<div class="linkst">
	<div class="inbox">
		<ul class="crumbs">
			...
			<li><span>»&#160;</span><a href="forums.php?id=<?php echo $cur_post['fid'] ?>"><?php echo pun_htmlspecialchars($cur_post['forum_name']) ?></a></li>
			<li><span>»&#160;</span><a href="posts.php?pid=<?php echo $post_id ?>#p<?php echo $post_id ?>"><?php echo pun_htmlspecialchars($cur_post['subject']) ?></a></li>
...
else if ($action === 'subscribe')
{
	...
	if ($topic_id)
	{
		...
		redirect('posts.php?id='.$topic_id, $lang_misc['Subscribe redirect']);
	}
	...
	if ($forum_id)
	{
		...
		redirect('forums.php?id='.$forum_id, $lang_misc['Subscribe redirect']);
	}
}
...
else if ($action === 'unsubscribe')
{
	...
	if ($topic_id)
	{
		...
		redirect('posts.php?id='.$topic_id, $lang_misc['Unsubscribe redirect']);
	}
	...
	if ($forum_id)
	{
		...
		redirect('forums.php?id='.$forum_id, $lang_misc['Unsubscribe redirect']);
	}
}
